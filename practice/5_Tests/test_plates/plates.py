...
def no_mid_num(s):
    for c in range(len(s)):
        if s[c].isdigit():
            return s[c:].isdigit()
            # This will return True if all characters after the first digit are digits.

    return True
    # This will return True if no digits exist (i.e. all characters are alpha)
...
def first_number_non_zero(s):
    for c in range(len(s)):
        if s[c].isdigit():
            return s[c] != "0"
            # This will return True if the first digit does not equal zero.
    return True
    # This will return True if no digits exist (i.e. all characters are alpha)
